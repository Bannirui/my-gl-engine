...
namespace Hazel
{
    ...
    ImGuiKey KeyCodeToImGuiKey(KeyCode code)
    {
        ...
        // A‑Z
        if (code >= Key::A && code <= Key::Z) return static_cast<ImGuiKey>(
            ImGuiKey_A + (static_cast<int>(code) - static_cast<int>(Key::A)));
        // 0‑9
        if (code >= Key::D0 && code <= Key::D9) return static_cast<ImGuiKey>(
            ImGuiKey_0 + (static_cast<int>(code) - static_cast<int>(Key::D0)));
        // F1‑F12
        if (code >= Key::F1 && code <= Key::F12) return static_cast<ImGuiKey>(
            ImGuiKey_F1 + (static_cast<int>(code) - static_cast<int>(Key::F1)));
        // 没有映射
